...
class VideoCrafterPipeline(AnimateDiffBasePipeline, TextualInversionLoaderMixin, LoraLoaderMixin, FromSingleFileMixin):
    ...
    def check_inputs(
        ...
        if num_frames <= 0:
            raise ValueError(f"`num_frames` has to be a positive integer but is {num_frames}.")
    ...
    def prepare_latents(
        ...
        shape = (
            batch_size, num_frames, num_channels_latents, height // self.vae_scale_factor, width // self.vae_scale_factor
        )
    ...
    def __call__(
        ...
        self.check_inputs(
            prompt, num_frames, height, width, callback_steps, negative_prompt, prompt_embeds, negative_prompt_embeds
        )
        ...
        # 7. Denoising loop
        clip_range_scheduler: ClipRangeScheduler = get_context_scheduler(
        ...
        batch_size = len(images) // num_frames
        if output_type == "pil":
            images = [
                images[i * num_frames : (i + 1) * num_frames]
                for i in range(batch_size)
            ]
        else:
            images = images.reshape(-1, num_frames, *images.shape[1:])
